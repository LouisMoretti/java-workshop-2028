...
public class Travel {
    public static void travelTo(Country source, Country destination) {
        System.out.printf("Boarding in %s, local date and time is: %s\n", source.countryName,
                ZonedDateTime.now(source.zoneId).format(DateTimeFormatter.RFC_1123_DATE_TIME));
        System.out.printf("Landing in %s, local date and time on arrival will be: %s\n", destination.countryName,
                ZonedDateTime.now(destination.zoneId).plusHours(source.travelTimes.get(destination.countryName)).format(DateTimeFormatter.RFC_1123_DATE_TIME));
    }
}
