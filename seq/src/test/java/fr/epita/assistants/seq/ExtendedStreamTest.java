...
public class ExtendedStreamTest {
    ...
    private static <TYPE> ExtendedStream<TYPE> ctor(final Collection<TYPE> values) {
    ...
    @Test
    public void toMapKeyValueFromList() {
        final List<DummyObject> dummies = List.of(dummy(1, "1"), dummy(2, "2"), dummy(2, "4"), dummy(3, "3"));
        assertTimeoutPreemptively(Duration.ofSeconds(10), () -> {
            final ExtendedStream<DummyObject> es = ctor(dummies);
            final Map<Integer, String> map = es.toMap(DummyObject::getId, DummyObject::getName);
            assertEquals(3, map.size());
            assertEquals("1", map.get(1));
            assertEquals("4", map.get(2));
    ...
    @Test
    public void toSumFromStream() {
        final Stream<Integer> gauss = IntStream.range(1, 101).boxed();
        assertTimeoutPreemptively(Duration.ofSeconds(10), () -> {
            final ExtendedStream<Integer> es = ctor(gauss);
            assertEquals(5050, es.mapToInt(Integer::intValue).sum());
        });
    }
    ...
    @Test
    public void leftAssociateWithStream() {
        assertTimeoutPreemptively(Duration.ofSeconds(10), () -> {
            final ExtendedStream<String> es = ctor("a", "b", "c");
    ...
    @Test
    public void rightAssociateWithStream() {
        assertTimeoutPreemptively(Duration.ofSeconds(10), () -> {
            final ExtendedStream<Integer> es = ctor(List.of(1, 2, 3, 4, 5));
            final List<Pair<Integer, String>> list = es.associate(ctor("a", "b", "c")).toList();

            assertEquals(3, list.size());
            assertEquals(new Pair<>(1, "a"), list.get(0));
            assertEquals(new Pair<>(2, "b"), list.get(1));
            assertEquals(new Pair<>(3, "c"), list.get(2));
        });
    }
    ...
    public void partition() {
        assertTimeoutPreemptively(Duration.ofSeconds(10), () -> {
            final ExtendedStream<Integer> es = ctor(IntStream.range(0, 8).boxed());
    ...
    public void print() {
        assertTimeoutPreemptively(Duration.ofSeconds(10), () -> {
            final ExtendedStream<Integer> es = ctor(Stream.of(1, 2, 3, 4, 5));

            final ByteArrayOutputStream out = new ByteArrayOutputStream();
            System.setOut(new PrintStream(out));
            es.print().map((e) -> e * e).forEach(System.out::println);
            assertEquals("""
                    1
                    1
                    2
    ...
    @Test
    public void nonTerminalPrint() {
        assertTimeoutPreemptively(Duration.ofSeconds(10), () -> {
            final ExtendedStream<String> es = ctor(Stream.of("You", "should", "not", "see", "that"));
